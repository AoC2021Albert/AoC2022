...
while not matchfound:
    ...
    while rockcangodown (chimney, rock, rockbottom):
        ...
        jet += 1
        jet %= len(jetpattern)
    initrockformation = ".".join((str(row) for row in chimney[rockbottom:towerheight+1]))
    see=f'{jet},{initrock},{initrockformation}'
    if see in saw:
        matchfound = True
...
print(heights[offset+postcycleoffset-1]+ cycleheight * ((ROUNDS-offset) // cyclesize))
